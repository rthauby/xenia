...
static void handle_O_option(char *z){
  output_name = (char *) malloc( lemonStrlen(z)+1 );
  if( output_name==0 ){
    memory_error();
  }
  lemon_strcpy(output_name, z);
}
...
int main(int argc, char **argv)
{
  ...
  static struct s_options options[] = {
    ...
    {OPT_FSTR, "o", (char*)handle_O_option, "Specify an output file path."},
...
to follow the previous rule.");
    ...
    case WAITING_FOR_DECL_KEYWORD:
      if( isalpha(x[0]) ){
        ...
        }else if( strcmp(x, "namespace_begin")==0 ){
          psp->declargslot = &(psp->gp->namespace_begin);
        }else if( strcmp(x, "namespace_end")==0 ){
          psp->declargslot = &(psp->gp->namespace_end);
...
PRIVATE char *file_makename(struct lemon *lemp, const char *suffix)
{
  ...
  char *outname = output_name ? output_name : lemp->filename;

  name = (char*)malloc( lemonStrlen(outname) + lemonStrlen(suffix) + 5 );
  if( name==0 ){
    fprintf(stderr,"Can't allocate space for a filename.\n");
    exit(1);
  }
  lemon_strcpy(name,outname);
  cp = strrchr(name,'.');
  if( cp ) *cp = 0;
...
{
  static char templatename[] = "lempar.cc";
  char buf[1000];
  FILE *in;
  char *tpltname;
  char *cp;
...
void ReportTable(
  ...
  if( lemp->namespace_begin ){
    tplt_print(out,lemp,lemp->namespace_begin,&lineno);
  }
  tplt_xfer(lemp->name,in,out,&lineno);
  ...
  ** empty actions. */
  ...
  if( lemp->namespace_end ){
    tplt_print(out,lemp,lemp->namespace_end,&lineno);
  }
